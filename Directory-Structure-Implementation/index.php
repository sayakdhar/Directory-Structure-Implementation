...
if($_GET['do'] == 'list') {
	if (is_dir($file)) {
		...
		foreach ($files as $entry) if (!e_i($entry, $all_shf, $h_p)) {
			$i = $directory . '/' . $entry;
			$stat = stat($i);
			$result[] = [
				'mtime' => $stat['mtime'],
				...
				'is_deleteable' => $allow_delete && ((!is_dir($i) && is_writable($directory)) ||
														(is_dir($i) && is_writable($directory) && rdel($i))),
...
function e_i($entry, $all_shf, $h_p) {
	if ($entry === basename(__FILE__)) {
		return true;
	}

	if (is_dir($entry) && !$all_shf) {
		return true;
	}
	foreach($h_p as $pattern) {
...
<?php endif; ?>
	function list() {
		...
		$.get('?do=list&file='+ hashval,function(data) {
			$tbody.empty();
			$('#breadcrumb').empty().html(rB(hashval));
			if(data.success) {
...
<footer>Frrole Project <a href="https://github.com/sayakdhar">--sayak</a></footer>
</body></html>
